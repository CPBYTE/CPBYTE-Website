...
const page = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      onOpen();
      try {
        const { data, error } = await supabase.from("secondyearform").insert([
          {
            name: values.name,
            email: values.email,
            id: values.id,
      ...
      <div className="max-w-screen-sm mx-auto my-5">
        <h2 className="max-w-lg  mx-3 mb-6 font-sans text-3xl  font-bold leading-none tracking-tight text-gray-900 sm:text-4xl ">
          ...
          <p className="text-sm font-medium">
            This form is for second year students only.
          </p>
        </h2>
        ...
        <form onSubmit={formik.handleSubmit} className="mx-2">
          ...
          <Input
            value="email"
            label="Your Email"
            placeHolder="varshneydev854@gmail.com"
          ...
          <Input
            value="linkedin"
            label="Linkedin Url"
            placeHolder="www.linkedin.com/in/varshneydev27/"
            formikTouched={formik.touched.linkedin}
            formikError={formik.errors.linkedin}
            formikChange={formik.handleChange}
            formikBlur={formik.handleBlur}
            formikValue={formik.values.linkedin}
            type="text"
          />
          ...
          <Input
            ...
            placeHolder="adityatyagi-av"
            formikTouched={formik.touched.github}
            formikError={formik.errors.github}
            formikChange={formik.handleChange}
            formikBlur={formik.handleBlur}
            formikValue={formik.values.github}
            type="text"
          />

          <Input
            value="link"
            label="Any other link you want to share"
            placeHolder="https://leetcode.com/Devvarshney/"
            formikTouched={formik.touched.link}
            formikError={formik.errors.link}
            formikChange={formik.handleChange}
            formikBlur={formik.handleBlur}
            formikValue={formik.values.link}
            type="text"
          />
